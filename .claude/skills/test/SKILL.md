...
**1つのメッセージ内で** 複数の Task tool 呼び出しを行い、ワーカーを並列起動する。
...
**重要**: `isolation: "worktree"` はworkerエージェント定義に含まれているため、Task tool呼び出し時に指定する必要はない。
