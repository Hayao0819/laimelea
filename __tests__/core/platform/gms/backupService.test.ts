...
describe("createGmsBackupService", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe("isAvailable", () => {
    it("should return true when signed in", async () => {
      ...
      const service = createGmsBackupService(auth);
  ...
  describe("getLastBackupTime", () => {
    it("should return modifiedTime as Unix ms", async () => {
      const auth = createMockAuthService("valid-token");
      const service = createGmsBackupService(auth);
      const modifiedTime = "2026-02-20T10:30:00.000Z";
