...
### カレンダーアクセス（全プラットフォーム共通）

全プラットフォーム（GMS/HMS/AOSP）で Android CalendarContract（カスタム Kotlin Turbo Module）を使用。`READ_CALENDAR` パーミッションのみで動作し、OAuth 認証やサインインは不要。端末のカレンダーアプリ（Google カレンダー等）に同期されたアカウントの予定を自動的に表示する。
