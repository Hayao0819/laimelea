...
| feature-dev プラグイン              | code-explorer / code-architect（オプション） |
...
`$ARGUMENTS` がテキスト記述や新規機能の場合、従来の Explore サブエージェントに加えて feature-dev プラグインのエージェントを活用できる:

- **code-explorer**: コードベースの網羅的な探索、影響範囲の分析
- **code-architect**: 既存アーキテクチャとの整合性を考慮した設計案の生成

Phase番号やファイルパスが指定された場合は従来の Explore が高速。テキスト記述の新規機能では code-explorer + code-architect の並列起動が有効。
