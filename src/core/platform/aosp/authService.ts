...
async function loadAuthState(): Promise<StoredAuthState | null> {
  const json = await AsyncStorage.getItem(STORAGE_KEYS.AOSP_AUTH_STATE);
  if (!json) {
    return null;
  }
...
async function saveAuthState(state: StoredAuthState): Promise<void> {
  await AsyncStorage.setItem(
    STORAGE_KEYS.AOSP_AUTH_STATE,
    JSON.stringify(state),
  );
}

async function clearAuthState(): Promise<void> {
  await AsyncStorage.removeItem(STORAGE_KEYS.AOSP_AUTH_STATE);
}
...
export function createAospAuthService(): PlatformAuthService {
  return {
    ...
    async signIn(): Promise<AuthResult> {
      const result = await authorize(AOSP_AUTH_CONFIG);
    ...
    async getAccessToken(): Promise<string | null> {
      ...
      try {
        const result = await refresh(AOSP_AUTH_CONFIG, {
